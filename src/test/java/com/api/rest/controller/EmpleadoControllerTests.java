...
@WebMvcTest // Va a servir para poder probar los controladores
public class EmpleadoControllerTests {

    @Autowired
    private MockMvc mockMvc; // Para probar peticicones HTTP

    @MockBean // Para agregar objeto simulado al contexto de la aplicación
    ...
    void testGuardarEmpleado() throws Exception {
        ...
        Empleado empleado = Empleado.builder()
                ...
                .email("c1@gmail.com")
                .build();
        given(empleadoService.saveEmpleado(any(Empleado.class))) // Guardar algun empleado
                .willAnswer((invocation) -> invocation.getArgument(0)); // Voy a responder con una función lambda, voy a obtener argumento 0

        //when: Cuando haga una petición POST
        ResultActions response = mockMvc.perform(post("/api/empleados")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(empleado)));// Convertir objeto en JSON

        //then
        response.andDo(print())
                .andExpect(status().isCreated()) // Código de estado sea creado
                .andExpect(jsonPath("$.nombre",is(empleado.getNombre()))) // (acceder atributo, nombreObjeto)
    ...
    void testObtenerEmpleadoPorId() throws Exception{
        ...
        //then
        response.andExpect(status().isOk())
                .andDo(print()) // Imprimir valores de empleados
    ...
    void testActualizarEmpleado() throws Exception{
        ...
        given(empleadoService.getEmpleadoById(empleadoId)).willReturn(Optional.of(empleadoGuardado));// Retornar empleado guardado
        given(empleadoService.updateEmpleado(any(Empleado.class))) // Actualizar empleado
                .willAnswer((invocation) -> invocation.getArgument(0));

        //when
        ResultActions response = mockMvc.perform(put("/api/empleados/{id}",empleadoId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(empleadoActualizado)));// Escribir como JSON

        //then - Resultados que estoy esperando
        response.andExpect(status().isOk())
                .andDo(print())
    ...
    void testEliminarEmpleado() throws Exception{
        //given
        long empleadoId = 1L;
        willDoNothing().given(empleadoService).deleteEmpleado(empleadoId);// No hare nada luego de eliminar un empleado
